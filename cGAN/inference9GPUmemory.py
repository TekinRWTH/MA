...
import configinference  # Assume DEVICE is defined here as a string (e.g., "cuda" or "cpu")
...
def inference_fn(gen, disc, bce, D_real_losses, D_fake_losses, G_losses, l1_loss, loader):
    ...
    inference_times = []
    fps_values = []
    gpu_memory = []  # NEW: Track GPU memory per batch

    for idx, (x, y) in enumerate(loop):  
        x, y = x.to(DEVICE), y.to(DEVICE)

        # NEW: Reset CUDA memory stats for all GPUs
        if DEVICE.type == 'cuda':
            for gpu_id in range(torch.cuda.device_count()):
                torch.cuda.reset_peak_memory_stats(gpu_id)

        # Timing with synchronization for CUDA
        ...
        fps_values.append(1 / elapsed_time if elapsed_time > 0 else 0)

        # NEW: Capture peak memory for all GPUs
        batch_gpu_memory = []
        if DEVICE.type == 'cuda':
            for gpu_id in range(torch.cuda.device_count()):
                mem = torch.cuda.max_memory_allocated(gpu_id)  # Bytes
                batch_gpu_memory.append(mem)
        gpu_memory.append(batch_gpu_memory)  # List of lists
        ...
        # Generator evaluation (autocast only for CUDA)
    ...
    # NEW: Add GPU memory columns dynamically
    data_dict = {
...
def main():
    # Initialize models on the configured device
